...
function playClueSequence() {
  ...
  for (let i = 0; i <= progress; i++) {
    // for each clue that is revealed so far, for each turn
...
function guess(btn) {
  ...
  if (btn == pattern[guessCounter]) {
    ...
    if (progress == guessCounter) {
      // if guesses == game progress or turn
      ...
      if (progress == pattern.length) {
        ...
        // guess correct and there are more turns left
        playClueSequence();
      } // if
    } else {
      // if there's more than one guess needed
      guessCounter++;
    } // if
  } else {
...
function playTone(btn, len) {
  // takes in button number and length (ms)
...
function startTone(btn) {
  // tone keeps playing until stopTone()
  if (!tonePlaying) {
